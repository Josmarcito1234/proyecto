...
if (isset($_GET['accion']) && isset($_GET['id'])) {
    ...
    if ($_GET['accion'] == 'agregar') {
        ...
        if (!$encontrado) {
            $_SESSION['carrito'][] = ['id_producto' => $id_producto, 'cantidad' =>
            1];
        }
                    ...
                    if (!empty($_SESSION['carrito'])) {
                        foreach ($_SESSION['carrito'] as $item) {
                            ...
                            if ($resultado->num_rows > 0) {
                                ...
                                $total += $subtotal;
                                echo '<tr><td>' . $producto['nombre'] . '</td><td>' . $item['cantidad'] . '</td><td>$' . number_format($producto['precio'], 2) .
                                    '</td><td>$' . number_format($subtotal, 2) . '</td><td> <a href="carrito.php?accion=eliminar&id=' .
                                    $item['id_producto'] . '" class="btn btn-danger btn-sm">Eliminar</a></td></tr>';
                            }
                        }
                    } else {
